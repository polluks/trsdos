...
PRBGN	JR	Z,$?2		;Go if PUT (output)
	JR	C,$?1		;Go if GET (input)

; CTL request
	LD	A,C
	OR	A
	JR	Z,$?4		;CTL 0 = status check
	JR	$?1		;Otherwise treat as GET
...
$?2	; Write character to printer via IEC serial
...
$?4	; Check if printer is present (attempt to talk)
	LD	A,IEC_DEV_PRINT
	OR	IEC_TALK
	CALL	IEC_ATN_ON
	CALL	IEC_BYTE_OUT
