...
;* through registers at _L_D600_0 (address) and _L_D601_0 (data).	*
;*								*
...
; Write to VDC register A with value in A
VDC_WRITE
	PUSH	BC
	LD	BC,VDC_ADDR
...
; Entry from SVC 15, _VDCTL
_VDCTL	JP	__VDCTL

; Main driver entry
DOBGN	LD	IX,DODATA_S
	JP	C,_L_0_GET_0	;Go on 'GET' request
	CALL	_L_0_0		;Handle cursor normalization
	PUSH	BC
	LD	A,C		;Get char to display
	BIT	4,(IX+DO_MASK)	;Display controls set?
	JR	NZ,_L_1A_131		;Go if so
	OR	A		;Char a 0?
	JP	Z,TGGLCTL	;Switch control display if so
	CP	20H		;Video control character?
	JP	C,DO_CONTROL	;Go if control char
_L_1A_131	CP	0C0H		;Tab or special?
...
DO_SCROLL
	...
	LD	A,' '
	VDC_SEL	VDC_ATTR
	LD	A,0		;Normal attribute
CLR_LAST
	PUSH	AF
	VDC_SEL	VDC_ATTR
	OUT	(C),A
	LD	A,' '
	VDC_SEL	VDC_SALOW	;Auto-increment
	OUT	(C),A
	POP	AF
	DJNZ	CLR_LAST
	POP	DE
...
CLREOL	PUSH	HL
	...
	LD	A,' '		;Space
CLREOL1
	VDC_SEL	VDC_SALOW	;Auto-increment write
	OUT	(C),A
	DJNZ	CLREOL1
	POP	BC
	POP	HL
...
;==============================================================================
; _VDCTL SVC Handler (video control functions)
;==============================================================================
__VDCTL
	...
	DEC	B
	JR	Z,_VDCTL3	;Function 3: Set cursor to H,L
	DEC	B
